...
trait Tables {
  ...
  lazy val schema = Array(Abstract.schema, Autoproperties.schema, Bulkdatagroups.schema, Data.schema, Datadependencies.schema, Datagroupings.schema, Diffs.schema, Grading.schema, Headers.schema, Matches.schema, Pdffonts.schema, Pdfmeta.schema, Pdftohtml.schema, Runids.schema, Runs.schema, Sentences.schema, Title.schema).reduceLeft(_ ++ _)
   ...
   *  @param bulkid Database column bulkID SqlType(BIGINT), PrimaryKey
  ...
  class Bulkdatagroups(_tableTag: Tag) extends Table[BulkdatagroupsRow](_tableTag, "BulkDataGroups") {
    ...
    def ? = (Rep.Some(bulkid), Rep.Some(dataid)).shaped.<>({r=>import r._; _1.map(_=> BulkdatagroupsRow.tupled((_1.get, _2.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column bulkID SqlType(BIGINT), PrimaryKey */
    val bulkid: Rep[Long] = column[Long]("bulkID", O.PrimaryKey)
   ...
   *  @param datatype Database column dataType SqlType(INT), Default(None)
   *  @param datatopic Database column dataTopic SqlType(VARCHAR), Length(45,true), Default(None) */
  ...
  class Datagroupings(_tableTag: Tag) extends Table[DatagroupingsRow](_tableTag, "DataGroupings") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(groupid), datatype, datatopic).shaped.<>({r=>import r._; _1.map(_=> DatagroupingsRow.tupled((_1.get, _2, _3)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column groupID SqlType(INT), PrimaryKey */
    val groupid: Rep[Int] = column[Int]("groupID", O.PrimaryKey)
    /** Database column dataType SqlType(INT), Default(None) */
    val datatype: Rep[Option[Int]] = column[Option[Int]]("dataType", O.Default(None))
    /** Database column dataTopic SqlType(VARCHAR), Length(45,true), Default(None) */
    val datatopic: Rep[Option[String]] = column[Option[String]]("dataTopic", O.Length(45,varying=true), O.Default(None))
  }
